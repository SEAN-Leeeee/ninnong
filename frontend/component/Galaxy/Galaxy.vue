...
const init = () => {
  ...
  function update(t) {
    animateId.value = requestAnimationFrame(update)
    ...
    const lerp = 0.05
    smoothMousePos.value.x += (targetMousePos.value.x - smoothMousePos.value.x) * lerp
    smoothMousePos.value.y += (targetMousePos.value.y - smoothMousePos.value.y) * lerp
    smoothMouseActive.value += (targetMouseActive.value - smoothMouseActive.value) * lerp
...
const destroy = () => {
  if (animateId.value) cancelAnimationFrame(animateId.value)
...
// React의 deps 재실행 동작을 맞추기 위해 주요 prop이 바뀌면 재초기화
watch(
    () => ({
      focal: props.focal,
    ...
    () => {
      // 완전 동일하게 React와 같이 언마운트 후 재마운트 느낌으로 재설정
      destroy()
      init()
    },
    { deep: true }
